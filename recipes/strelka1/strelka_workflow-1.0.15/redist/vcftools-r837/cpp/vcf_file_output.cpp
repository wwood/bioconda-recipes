...
void vcf_file::output_frequency(const string &output_file_prefix, bool output_counts, bool suppress_allele_output, bool derived)
{
...
void vcf_file::calc_hap_r2(vcf_entry &e, vcf_entry &e2, const vector<bool> &include_geno1, const vector<bool> &include_geno2, double &r2, double D, double &Dprime, int &chr_count)
{
...
void vcf_file::calc_r2_em(vcf_entry &e, vcf_entry &e2, const vector<bool> &include_geno1, const vector<bool> &include_geno2, double &r2, int &indv_count)
{
	r2 = 0;
	indv_count = 0;
	pair<int, int> geno1, geno2;
...
void vcf_file::calc_geno_r2(vcf_entry &e, vcf_entry &e2, const vector<bool> &include_geno1, const vector<bool> &include_geno2, double &r2, int &indv_count)
{
	double X=0, X2=0, Y=0, Y2=0, XY=0;
	double sx, sy;
	indv_count = 0;
...
void vcf_file::output_haplotype_r2(const string &output_file_prefix, int snp_window_size, int snp_window_min, int bp_window_size, int bp_window_min, double min_r2)
{
...
void vcf_file::output_genotype_r2(const string &output_file_prefix, int snp_window_size, int snp_window_min, int bp_window_size, int bp_window_min, double min_r2)
{
...
void vcf_file::output_interchromosomal_haplotype_r2(const string &output_file_prefix, double min_r2)
{
	// Output pairwise LD statistics, using genotype r^2. This is the same formula as used by PLINK, and is basically the squared
	// correlation coefficient between genotypes numbered as 0, 1, 2.
	if ((has_genotypes == false) | (N_kept_individuals() == 0))
...
void vcf_file::output_haplotype_r2_of_SNP_list_vs_all_others(const string &output_file_prefix, const string &positions_file, double min_r2)
{
	if ((has_genotypes == false) | (N_kept_individuals() == 0))
			LOG.error("Require Genotypes in VCF file in order to output LD Statistics.");
...
void vcf_file::output_genotype_r2_of_SNP_list_vs_all_others(const string &output_file_prefix, const string &positions_file, double min_r2)
{
	if ((has_genotypes == false) | (N_kept_individuals() == 0))
			LOG.error("Require Genotypes in VCF file in order to output LD Statistics.");
...
void vcf_file::output_hapmap_fst(const string &output_file_prefix, const vector<string> &indv_files)
{
...
void vcf_file::output_weir_and_cockerham_fst(const string &output_file_prefix, const vector<string> &indv_files)
...
void vcf_file::output_windowed_weir_and_cockerham_fst(const string &output_file_prefix, const vector<string> &indv_files, int fst_window_size, int fst_window_step)
{
...
void vcf_file::output_windowed_hapmap_fst(const string &output_file_prefix, const vector<string> &indv_files, int fst_window_size, int fst_window_step)
{
	if (fst_window_size <= 0)
		return;
...
void vcf_file::output_windowed_nucleotide_diversity(const string &output_file_prefix, int window_size, int window_step)
{
...
void vcf_file::output_indel_hist(const string &output_file_prefix)
{
	string vcf_line;
	vcf_entry e(N_indv);
	string allele;
