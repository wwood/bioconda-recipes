...
int main(int argc, char *argv[])
{
	...
	vcf_file vcf(params.vcf_filename, params.vcf_compressed, params.chrs_to_keep, params.chrs_to_exclude, params.force_write_index);
	...
	{	// Merge files - cannot be run with other output options.
		vcf_file vcf_diff(params.diff_file, params.diff_file_compressed, params.chrs_to_keep, params.chrs_to_exclude, params.force_write_index);
	...
	if (params.output_freq == true) vcf.output_frequency(params.output_prefix, false, params.suppress_allele_output, params.derived);
	if (params.output_counts == true) vcf.output_frequency(params.output_prefix, true, params.suppress_allele_output, params.derived);
	...
	if (params.output_geno_rsq == true) vcf.output_genotype_r2(params.output_prefix, params.ld_snp_window_size, params.ld_snp_window_min, params.ld_bp_window_size, params.ld_bp_window_min, params.min_r2);
	if (params.output_interchromosomal_hap_rsq == true) vcf.output_interchromosomal_haplotype_r2(params.output_prefix, params.min_r2);
	if (params.output_interchromosomal_geno_rsq == true) vcf.output_interchromosomal_genotype_r2(params.output_prefix, params.min_r2);
	if (params.output_hap_rsq == true) vcf.output_haplotype_r2(params.output_prefix, params.ld_snp_window_size, params.ld_snp_window_min, params.ld_bp_window_size, params.ld_bp_window_min, params.min_r2);
	if (params.hap_rsq_position_list != "") vcf.output_haplotype_r2_of_SNP_list_vs_all_others(params.output_prefix, params.hap_rsq_position_list, params.min_r2);
	if (params.geno_rsq_position_list != "") vcf.output_genotype_r2_of_SNP_list_vs_all_others(params.output_prefix, params.geno_rsq_position_list, params.min_r2);
	...
	if (params.output_BEAGLE_genotype_likelihoods_GL == true) vcf.output_BEAGLE_genotype_likelihoods(params.output_prefix, 0);
	if (params.output_BEAGLE_genotype_likelihoods_PL == true) vcf.output_BEAGLE_genotype_likelihoods(params.output_prefix, 1);
	if (params.output_as_ldhat_unphased == true) vcf.output_as_LDhat_unphased(params.output_prefix);
	if (params.output_as_ldhat_phased == true) vcf.output_as_LDhat_phased(params.output_prefix);
	...
	if (params.pi_window_size > 0) vcf.output_windowed_nucleotide_diversity(params.output_prefix, params.pi_window_size, params.pi_window_step);
	...
	{
		if (params.hapmap_fst_populations.size() > 0) vcf.output_windowed_hapmap_fst(params.output_prefix, params.hapmap_fst_populations, params.fst_window_size, params.fst_window_step);
		if (params.weir_fst_populations.size() > 0) vcf.output_windowed_weir_and_cockerham_fst(params.output_prefix, params.weir_fst_populations, params.fst_window_size, params.fst_window_step);
	}
	if (params.output_indel_hist == true) vcf.output_indel_hist(params.output_prefix);
