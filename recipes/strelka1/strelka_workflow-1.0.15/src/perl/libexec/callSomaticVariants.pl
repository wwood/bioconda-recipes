...
my $somRegionFile='somatic.callable.region.bed';
...
if(exists($useroptions->{isWriteCallableRegion}) && ($useroptions->{isWriteCallableRegion} > 0)) {
    $cmd .= " --somatic-callable-region-file " . File::Spec->catfile($binDir,$somRegionFile);
}
