...
checkFile($bgzipBin,"bgzip binary");
...
sub consolidateBed() {
    my $fileName = 'somatic.callable.region.bed';

    my $reDir = File::Spec->catdir($outDir,'regions');
    checkMakeDir($reDir);
    ...
    for my $chrom (@chromOrder) {
        ...
        for my $binId (@$binList) {
            my $binDir = File::Spec->catdir($chromDir,'bins',$binId);
            my $rbedFile = File::Spec->catfile($binDir,$fileName);
            checkFile($rbedFile,"bin callable region file");

            push @bedList,$rbedFile;
        }
    }

    return unless(scalar(@bedList));

    my $outputFileName = $fileName . ".gz";
    my $allFile = File::Spec->catfile($reDir,$outputFileName . $itag);

    open(my $bgFH, "| $bgzipBin -c > $allFile")
          || errorX("Failed to open bgzip stream to: '$allFile'");

    my $bedCount=0;
    for my $bedFile (@bedList) {
        $bedCount++;
        open(my $bedFH, "< $bedFile")
              || errorX("Failed to open bed file: '$bedFile'");

        for my $line (<$bedFH>) {
            next if (($line=~/StrelkaCallableSites/) && ($bedCount>1));
            print $bgFH $line;
        }
        close($bedFH) || errorX("Failed to close bed file");
    }
    close($bgFH) || errorX("Failed to close bgzip stream");

    my $allFileFinished = File::Spec->catfile($reDir,$outputFileName);
    checkMove($allFile,$allFileFinished);

    # for now don't remove all the bin regions...
    # unlink(@bedList);
}



if(exists($userconfig->{isWriteCallableRegion}) && ($userconfig->{isWriteCallableRegion} > 0)) {
    consolidateBed();
}
