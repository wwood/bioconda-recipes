...
# max indel interrupted hompolymer length:
...
sub filterIndelFileList(\@$$$) {
    ...
    for my $ifile (@$ifiles) {
        ...
        if($is_first) {
            ...
            add_vcf_filter($vcf,$iHpolFiltId,"Indel overlaps an interrupted homopolymer longer than ${indelMaxIntHpolLength}x in the reference sequence");
